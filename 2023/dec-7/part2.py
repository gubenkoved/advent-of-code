...
class Hand:
    ...
    def card_strengths(self):
        return tuple(CARD_ORDER.index(c) for c in self.raw)


def best_joker_expansion(cards):
    best_rank = None
    best_hand = None

    def process(cur_cards, idx):
        nonlocal best_rank
        nonlocal best_hand

        # end condition
        if idx == 5:
            hand = Hand(cards=cur_cards, raw=cards)
            hand_rank_tuple = hand.rank_tuple()
            if best_rank is None or hand_rank_tuple > best_rank:
                best_rank = hand_rank_tuple
                best_hand = hand
            return

        if cards[idx] != 'J':
            process(cur_cards + [cards[idx]], idx + 1)
        else:  # joker!
            # try all the cards
            for card in CARD_ORDER[1:]:
                process(cur_cards + [card], idx + 1)
